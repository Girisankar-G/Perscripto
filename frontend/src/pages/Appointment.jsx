...
const Appointment = () => {
  ...
  useEffect(() => {
    ...
    dayScrollRef.current?.addEventListener('wheel', onWheelDay, { passive: false })
    timeScrollRef.current?.addEventListener('wheel', onWheelTime, { passive: false })
  ...
  return docInfo && (
    <div className='p-4'>
      <div className='flex flex-col sm:flex-row gap-4'>
        <div>
          <img src={docInfo.image} alt="Doctor" className='bg-[#5f6fff] w-full sm:max-w-72 rounded-lg' />
        </div>
        <div className='flex-1 border border-gray-400 rounded-lg p-8 py-7 bg-white mx-2 sm:mx-0 mt-[-80px] sm:mt-0'>
          ...
          <p className='text-gray-500 font-medium mt-4'>
            Appointment fee: <span className='text-gray-600'>{currencySymbol} {docInfo.fees * 10}</span>
          </p>
        </div>
      </div>
      ...
      <div className='mt-6 sm:ml-72 sm:pl-4 font-medium text-gray-700'>
        ...
        {/* Days */}
        <div ref={dayScrollRef} className='flex gap-3 items-center w-full overflow-x-auto mt-4 px-2'>
          {
            docSlots.map((item, index) => {
              ...
              return (
                <div
                  ...
                  key={index}
                  className={`text-center py-6 min-w-16 rounded-full cursor-pointer flex-shrink-0 ${slotIndex === index ? 'bg-[#5f6fff] text-white' : 'border border-gray-200 text-gray-600'}`}
                >
        ...
        {/* Time slots */}
        <div ref={timeScrollRef} className='flex items-center gap-3 w-full overflow-x-auto mt-4 px-2'>
          {
            docSlots.length > 0 && docSlots[slotIndex]?.length > 0 ? docSlots[slotIndex].map((item, index) => (
              ...
              <p className="mt-2 text-[20px] text-gray-400">No slots available</p>
            )
          }
        </div>
        <button className="bg-[#5f6fff] mt-5 text-white cursor-pointer text-sm font-light px-14 py-3 rounded-full 
